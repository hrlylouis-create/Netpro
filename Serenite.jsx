...
import hero from './assets/hero2-product.png'

const faqItems = [
  { q: "À quelle fréquence ont lieu les sessions ?", a: "Les sessions se déroulent selon la fréquence choisie : hebdomadaire, bi-mensuelle ou mensuelle." },
  { q: "Ai-je toujours le même professionnel ?", a: "Oui, vous bénéficiez d'un professionnel attitré pour un résultat constant et une relation de confiance." },
  { q: "Puis-je mettre mon abonnement en pause ?", a: "Oui, vous pouvez mettre votre abonnement en pause à tout moment depuis votre espace client." },
  { q: "Comment résilier mon abonnement ?", a: "La résiliation est possible à tout moment, sans frais ni engagement minimum." },
  { q: "Comment est calculé le prix ?", a: "Le tarif est de 31,90€/h, soit 15,95€/h après crédit d'impôt (50% pour les particuliers)." },
]
...
function Serenite() {
  ...
  return (
    <div className="mr-page mr-page--serenite">
          ...
          {subbarVisible ? (
            <>
              <div className="mr-subbar-priceblock">
                ...
                <span className="mr-subbar-amount">{apresImpot ? '15,95€/H' : '31,90€/H'}</span>
              </div>
              <CTAButton color="green" className="cta-btn--sm" onClick={() => navigate('/souscription/menage-regulier')}>M'ABONNER</CTAButton>
            </>
          ) : (
            <>
      ...
      <section className="mr-hero mrr-hero" ref={heroRef}>
        <div className="mr-hero-content">
          <div className="mrr-badge-populaire">POPULAIRE</div>
          <span className="mr-prix-barre" style={{ visibility: apresImpot ? 'visible' : 'hidden' }}>31,90€/h</span>
          <div className="mr-titre-row">
            <span className="mr-nav-arrow" onClick={() => setTimeout(() => navigate('/menage-rapide'), 180)} style={{ cursor: 'pointer' }}>‹</span>
            <h1 className="mr-titre">SÉRÉNITÉ</h1>
            <span className="mr-nav-arrow" onClick={() => setTimeout(() => navigate('/grand-menage'), 180)} style={{ cursor: 'pointer' }}>›</span>
          </div>
          <div className="mr-prix-principal">{apresImpot ? '15,95€/H' : '31,90€/H'}</div>
          <span className="mrr-sans-engagement">Sans engagement</span>
          ...
          <CTAButton ref={ctaRef} color="green" onClick={() => navigate('/souscription/menage-regulier')} className="mr-hero-btn">M'ABONNER</CTAButton>
        </div>
        <div className="mr-hero-img-wrap mrr-hero-img-wrap">
      ...
      <section className="mr-valeurs" ref={valeursRef}>
        <h2 className={`mr-valeurs-titre${fu(valeursInView, valeursAnimDone)}`}>
          FAITES <span className="mot-vert">TOUJOURS</span> LE BON CHOIX
        </h2>
        <p className={`mr-valeurs-sous${fu(valeursInView, valeursAnimDone)}`} style={td(valeursAnimDone, '0.1s')}>Abonnez-vous dès maintenant</p>
        ...
        <div className="mr-valeurs-grid-wrap">
          <div className="mr-valeurs-grid">
            <div className={`mr-valeur-card${fu(valeursInView, valeursAnimDone)}`} style={td(valeursAnimDone, '0.15s')}>
              <h3 className="mr-valeur-title"><span className="mot-vert">TOUJOURS</span> NICKEL</h3>
              <p>Des sessions régulières, des résultats nets, et un niveau d'exigence toujours plus élevé</p>
            </div>
            <div className={`mr-valeur-card${fu(valeursInView, valeursAnimDone)}`} style={td(valeursAnimDone, '0.27s')}>
              <h3 className="mr-valeur-title"><span className="mot-rose">TOUJOURS</span> SEREIN</h3>
              <p>Un professionnel vérifié attitré, des accès sécurisés, et zéro mauvaise surprise</p>
            </div>
            <div className={`mr-valeur-card${fu(valeursInView, valeursAnimDone)}`} style={td(valeursAnimDone, '0.39s')}>
              <h3 className="mr-valeur-title"><span className="mot-vert">TOUJOURS</span> LIBRE</h3>
              <p>Adaptez, arrêtez ou mettez en pause votre abonnement facilement</p>
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="mr-options" ref={optionsRef}>
        ...
        <div className="mr-options-grid">
          {[
            ...
            { title: 'REPASSAGE', prix: '3€/H', desc: 'Repassage et pliage du linge', offert: false, icon: '👕' },
            { title: 'PRODUITS', prix: '3€/H', desc: 'Fourniture des produits ménagers', offert: false, icon: '🧴' },
            { title: 'PLANTES', prix: '2€', desc: 'Arrosage des fleurs et plantes', offert: true, icon: '🪴' },
          ].map((item, i) => (
