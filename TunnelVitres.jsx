...
const FORFAITS = [
  { val: 'S', label: 'Forfait S', prix: 39, hint: <>Idéal pour un <strong>appartement</strong> jusqu'à 50m²</> },
  { val: 'M', label: 'Forfait M', prix: 59, hint: <>Idéal pour une <strong>maison</strong> jusqu'à 100m²</> },
  { val: 'L', label: 'Forfait L', prix: 79, hint: <>Idéal pour une <strong>grande maison</strong> ou un duplex</> },
]
...
export default function TunnelVitres() {
  ...
  useEffect(() => {
    ...
    function handlePopState(e) {
      ...
      if (s != null) { setStep(s); setRecapExpanded(s === 6) }
    }
  ...
  function goNext() {
    ...
    const next = step + 1
    setStep(next)
    if (next === 6) setRecapExpanded(true)
  ...
  function goRestart() {
    ...
    setForfait(null); setSelectedDate(null); setCreneaux([])
    setRemiseCles(null); setCommentaire(''); setRecapExpanded(false)
    window.history.replaceState({ step: 1 }, '')
  }

  const selectedForfait = FORFAITS.find(f => f.val === forfait)
  const prixBase = selectedForfait?.prix ?? 0
  const avanceDiscount = avanceImmediate ? prixBase * 0.5 : 0
  const total = forfait ? prixBase - avanceDiscount : null
  ...
  const progress = step >= 6 ? 100 : (step / TOTAL_STEPS) * 100
  ...
  const renderStep2 = () => (
    <>
      <h2 className="smr-title">QUEL FORFAIT VITRES ?</h2>
      <p className="smr-sub">Choisissez le forfait adapté à <strong>la taille de votre logement</strong></p>
      <div className="smr-freq-list">
        {FORFAITS.map(f => (
          <button key={f.val} className={`smr-freq-btn${forfait === f.val ? ' selected' : ''}`} onClick={() => setForfait(f.val)}>
            <span>{f.label}</span>
            <span style={{ fontWeight: 700, color: '#F4824A' }}>{f.prix}€</span>
          </button>
        ))}
      </div>
      {selectedForfait && <p className="smr-hint">{selectedForfait.hint}</p>}
    </>
  )
  ...
  return (
    <div className="smr-page smr-page--integral">
      ...
      {recapExpanded && <div className="smr-outside-tap" onClick={() => step === 6 ? goPrev() : setRecapExpanded(false)} />}
      ...
      <div className="smr-nav">
        <div className="smr-nav-inner">
          ...
          {step < 6 && <CTAButton color="orange" onClick={goNext} disabled={!canNext}>SUIVANT</CTAButton>}
          {step > 1 && (
          ...
          <div className="smr-mini-ticket">
            ...
            <div className="smr-mini-ticket-row">
              <div className="smr-mini-ticket-left">
                ...
                <span className="smr-mini-ticket-detail">{forfait && selectedDate ? `Forfait ${forfait} · ${dateText}` : 'Vitres'}</span>
              </div>
          ...
          <div className="smr-ticket-wrapper">
            <div className="smr-ticket">
              ...
              <div className="smr-ticket-body">
                ...
                <p className="smr-ticket-subtitle">Réservation Vitres</p>
                <div className="smr-ticket-sep" />
                <div className="smr-ticket-section">
                  <div className="smr-ticket-row"><span>Forfait</span><span>{selectedForfait ? `${selectedForfait.label} (${selectedForfait.prix}€)` : '—'}</span></div>
                ...
                <div className="smr-ticket-sep" />
                <div className="smr-ticket-section">
                  <div className="smr-ticket-row"><span>Forfait {forfait}</span><span>{prixBase > 0 ? `${fmt(prixBase)} €` : '—'}</span></div>
                  {avanceImmediate && forfait && <div className="smr-ticket-row smr-ticket-row--green"><span>Avance immédiate 50%</span><span>−{fmt(avanceDiscount)} €</span></div>}
                </div>
                ...
                <div className="smr-ticket-toggle">
                  ...
                  <div className="smr-toggle-row">
                    <span className={avanceImmediate ? 'smr-taux-barre' : 'smr-taux-reduit'}>{prixBase > 0 ? fmt(prixBase) : '—'}€</span>
                    <button className={`smr-toggle${avanceImmediate ? ' on' : ''}`} onClick={() => setAvanceImmediate(v => !v)} />
                    <span className={avanceImmediate ? 'smr-taux-reduit' : 'smr-taux-barre'}>{prixBase > 0 ? fmt(prixBase / 2) : '—'}€</span>
                  </div>
                </div>
                <div className="smr-ticket-sep" />
                <div className="smr-ticket-comment">
                  <p className="smr-ticket-comment-label">Note pour le pro</p>
                  <textarea className="smr-ticket-comment-area" placeholder="Type de vitres, accès particulier..." value={commentaire} onChange={e => setCommentaire(e.target.value)} rows={3} />
                </div>
                <div className="smr-ticket-sep" />
